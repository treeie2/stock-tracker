...
async function fetchStockPrice(code) {
  ...
  console.log(`Testing for code: ${code} -> ${tsCode}`);
  ...
  try {
    const response = await fetch(TUSHARE_URL, {
      ...
      body: JSON.stringify({
        ...
        fields: 'close,pct_chg,pe_ttm,total_mv'
      })
    });
    ...
    if (result.data && result.data.items && result.data.items.length > 0) {
      ...
      const price = item[0] ? item[0].toFixed(2).toString() : '---';
      const pct_chg = item[1] !== null ? (item[1] >= 0 ? `+${item[1].toFixed(2)}%` : `${item[1].toFixed(2)}%`) : '0.00%';
      const peRatio = item[2] ? item[2].toFixed(2).toString() : '---';
      const marketCap = item[3] ? `${(item[3] / 10000).toFixed(2)}亿` : '---';
