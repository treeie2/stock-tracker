...
const STOCK_DB_PATH = path.join(__dirname, '../src/stockDatabase.ts');
...
3. 每个股票对象必须包含以下字段：
   ...
   - stockCode: 6位数字股票代码（如 300593、002460）
     ...
     * 如果文本中没有股票代码，设置为 ""
   - title: 研报标题
   - logic: 投资逻辑/核心观点（总结为100字以内的精炼短句）
...
5. 确保每个股票都有独立的 stockCode 和 stockName，不要合并不同股票的信息`;
    ...
    try {
        ...
        if (error instanceof SyntaxError && typeof rawContent !== 'undefined') {
            console.log("Raw Content (First 100):", rawContent.substring(0, 100));
            console.log("Raw Content (Last 100):", rawContent.substring(rawContent.length - 100));
        }
        return [];
    }
}
...
function enrichStockCodes(extractedData) {
    ...
    try {
        ...
        return extractedData.map(item => {
            ...
            // Clean up code if it's not empty
            if (code) {
            ...
            if (!code || code.length !== 6) {
                const foundCode = stockMap.get(item.stockName);
                if (foundCode) {
                    item.stockCode = foundCode;
                    console.log(`Mapped ${item.stockName} -> ${foundCode}`);
                } else {
...
export async function scrapeArticle(url) {
    ...
    try {
        ...
        await page.waitForFunction(() => {
            return document.querySelector('#js_content') || document.querySelector('.rich_media_content');
        }, { timeout: 10000 });
    } catch (e) {
        console.log("Could not find content selector, trying body...");
    }

    content = await page.evaluate(() => {
        const element = document.querySelector('#js_content') || document.querySelector('.rich_media_content') || document.body;
    ...
    if (content.length > 5000) {
        ...
        console.log(`Truncated to 5000 chars for AI processing.`);
    }
    ...
    if (!extractedBuffer || extractedBuffer.length === 0) {
        ...
        finalData = enrichStockCodes(extractedBuffer);
        // Add source URL to the data
        finalData = finalData.map(item => ({ ...item, sourceUrl: url }));
    }
    ...
    (async () => {
        ...
        if (url) {
            try {
                ...
                console.log(`\nSuccess! Saved ${data.length} records to ${OUTPUT_FILE}`);
            } catch (err) {
                console.error("Fatal Error in scrapeArticle:", err);
                process.exit(1);
            }
        } else {
            console.log("Please provide a URL.");
            // Interactive mode test could go here if needed
        }
    })();
}
